...
export const persistAvatarDataUrl = async (buffer: Buffer): Promise<string> => {
  const optimized = await sharp(buffer).rotate().resize(512, 512, { fit: 'cover' }).jpeg({ quality: 82 }).toBuffer();
