...
const seed = async () => {
  ...
  // Build a real reporting line so the Manager portal "My Team" and team-scoped
  // attendance/leave views (and the e2e approval flow) have direct reports.
  const managerReportIds = new Set<string>([String(employeePortalUser._id)]);
  employees
    .filter(
      (item) =>
        String(item.department) === String(engineeringDepartment._id) &&
        String(item._id) !== String(managerEmployee._id),
    )
    .slice(0, 8)
    .forEach((item) => managerReportIds.add(String(item._id)));
  managerReportIds.delete(String(managerEmployee._id));
  await EmployeeModel.updateMany({ _id: { $in: [...managerReportIds] } }, { reportingTo: managerEmployee._id });
    ...
    for (const employee of employees) {
      ...
      for (const weekStart of attendanceWeeks) {
        for (let day = 0; day < 5; day += 1) {
          ...
          checkIn.setHours(10, faker.number.int({ min: 0, max: 25 }), 0, 0);
          const checkOut = new Date(date);
          checkOut.setHours(18, faker.number.int({ min: 0, max: 45 }), 0, 0);
          const lateMinutes = Math.max(checkIn.getHours() * 60 + checkIn.getMinutes() - (10 * 60), 0);
          const overtimeHours =
            checkOut.getHours() * 60 + checkOut.getMinutes() > 18 * 60
              ? Number((((checkOut.getHours() * 60 + checkOut.getMinutes()) - 18 * 60) / 60).toFixed(2))
              : 0;
