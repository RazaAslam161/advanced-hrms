...
export const syncDefaultOfficeShift = async (): Promise<void> => {
  const existing = await ShiftModel.findOne({ code: DEFAULT_SHIFT.code });
  if (existing) {
    await ShiftModel.updateOne({ _id: existing._id }, { $set: DEFAULT_SHIFT });
    return;
  }

  await ShiftModel.updateMany({}, { isDefault: false });
  await ShiftModel.create(DEFAULT_SHIFT);
};

const resolveDefaultShift = async () => {
  let shift = await ShiftModel.findOne({ isDefault: true });
  if (!shift) {
    shift = await ShiftModel.create(DEFAULT_SHIFT);
  }
...
export class AttendanceService {
  static async checkIn(userId: string, input: { lat: number; lng: number; timestamp: string }) {
    ...
    const timezone = employee.timezone || DEFAULT_OFFICE_TIMEZONE;
    const minutesNow = getMinutesInTimezone(now, timezone);
    const shiftStart = parseTimeToMinutes(shift.startTime);
    const { status, lateMinutes } = resolveAttendanceStatus({
      minutesIn: minutesNow,
    ...
    getSocket()?.emit('attendance:presence', {
  ...
  static async checkOut(userId: string, input: { lat: number; lng: number; timestamp: string }) {
    ...
    const shift = record.shiftId ? await ShiftModel.findById(record.shiftId) : await resolveDefaultShift();
    if (!shift) {
      throw new AppError('Shift configuration not found', 500);
    }

    const checkOut = new Date(input.timestamp);
    const timezone = employee.timezone || DEFAULT_OFFICE_TIMEZONE;
    ...
    record.overtimeHours = resolveOvertimeHours({
      minutesOut: getMinutesInTimezone(checkOut, timezone),
