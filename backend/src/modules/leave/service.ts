...
export class LeaveService {
  ...
  static async apply(
    ...
    if (requester.role === 'superAdmin') {
      throw new AppError('Super Admin accounts do not submit leave requests', 403);
    }

    let employee;
    if (requester.role === 'admin' && payload.employeeId) {
  ...
  static async approve(
    ...
    if (
      ...
      (request.status === 'pendingHR' || request.status === 'pendingManager')
    ) {
      request.approvals = request.approvals ?? {};
      request.status = 'approved';
      if (!request.approvals.managerApprovedBy) {
        request.approvals.managerApprovedBy = approver.userId as never;
        request.approvals.managerApprovedAt = new Date();
      }
