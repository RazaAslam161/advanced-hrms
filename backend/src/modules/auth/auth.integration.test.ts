...
describe('auth integration', () => {
  ...
  it('invalidates previous access tokens after account status changes', async () => {
    const { user: superAdmin } = await createUserWithEmployee(ctx, {
      role: 'superAdmin',
      email: 'authority@metalabstech.test',
    });
    const account = await createUserWithEmployee(ctx, {
      role: 'employee',
      email: 'token.version@metalabstech.test',
    });
    const oldToken = createAccessToken(ctx, account.user);

    const deactivateResponse = await request(ctx.app)
      .patch(`/api/v1/auth/users/${account.user.id}`)
      .set(bearerHeader(ctx, superAdmin))
      .send({ isActive: false });
    expect(deactivateResponse.status).toBe(200);

    const reactivateResponse = await request(ctx.app)
      .patch(`/api/v1/auth/users/${account.user.id}`)
      .set(bearerHeader(ctx, superAdmin))
      .send({ isActive: true });
    expect(reactivateResponse.status).toBe(200);

    const response = await request(ctx.app).get('/api/v1/employees/me').set('Authorization', `Bearer ${oldToken}`);
    expect(response.status).toBe(401);
    expect(response.body.message).toBe('Session is no longer valid');
  });

  it('removes company users from the access registry and blocks self-removal', async () => {
    const { user: superAdmin } = await createUserWithEmployee(ctx, {
      role: 'superAdmin',
      email: 'remove.super@metalabstech.test',
    });
    const account = await createUserWithEmployee(ctx, {
      role: 'employee',
      email: 'remove.target@metalabstech.test',
    });

    const selfRemove = await request(ctx.app)
      .delete(`/api/v1/auth/users/${superAdmin.id}`)
      .set(bearerHeader(ctx, superAdmin));

    expect(selfRemove.status).toBe(400);
    expect(selfRemove.body.message).toBe('You cannot remove your own account');

    const removeResponse = await request(ctx.app)
      .delete(`/api/v1/auth/users/${account.user.id}`)
      .set(bearerHeader(ctx, superAdmin));

    expect(removeResponse.status).toBe(200);
    expect(removeResponse.body.data.removed).toBe(true);

    const listResponse = await request(ctx.app).get('/api/v1/auth/users').set(bearerHeader(ctx, superAdmin));
    expect(listResponse.status).toBe(200);
    expect(listResponse.body.data.some((user: { id: string }) => user.id === account.user.id)).toBe(false);
  });
