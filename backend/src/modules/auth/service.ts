...
export const syncRolePermissions = async (): Promise<void> => {
  await UserModel.updateMany({ role: 'recruiter' }, { $addToSet: { permissions: 'recruitment.pipeline' } });
  await UserModel.updateMany({ role: 'employee' }, { $addToSet: { permissions: { $each: ['pulse.read'] } } });
...
const normalizeNameParts = (user: {
  ...
  const localPart = user.email.split('@')[0] ?? 'meta.user';
  ...
  return {
    firstName: firstName || segments[0] || 'Meta',
    lastName: lastName || segments[1] || 'User',
  };
};
...
const ensureEmployeeProfile = async (
  ...
  const normalizedNames = normalizeNameParts(user);
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      await EmployeeModel.create({
        ...
        displayName: `${normalizedNames.firstName} ${normalizedNames.lastName}`,
        email: user.email,
        department,
...
export class AuthService {
  ...
  static async listUsers() {
    const deletedUserIds = await EmployeeModel.find({ isDeleted: true }).distinct('userId');
    const users = await UserModel.find({ _id: { $nin: deletedUserIds } })
  ...
  static async removeUser(actorUserId: string, actorRole: Role, userId: string) {
    ...
    if (user.role === 'superAdmin') {
      throw new AppError('Super Admin accounts cannot be removed from the company registry', 403);
    }

    if (actorRole === 'admin' && user.role === 'admin') {
      throw new AppError('HR Admin cannot remove other HR accounts', 403);
    }

    const employee = await EmployeeModel.findOne({ userId: user._id, isDeleted: false });
    if (employee) {
      await EmployeeService.remove(employee.id, actorUserId);
    } else {
      user.isActive = false;
      user.tokenVersion += 1;
      await user.save();
    ...
    return { removed: true, userId: user.id };
  }
