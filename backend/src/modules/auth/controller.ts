...
export const removeUser = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const result = await AuthService.removeUser(req.user!.userId, req.user!.role, String(req.params.id));
  res.json(sendSuccess('User removed from company registry successfully', result));
});
