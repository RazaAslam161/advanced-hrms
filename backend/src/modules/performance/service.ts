...
export class PerformanceService {
  ...
  static async submitFeedback(userId: string, payload: Record<string, unknown>) {
    const reviewer = await EmployeeModel.findOne({ userId, isDeleted: false });
    if (!reviewer) {
      throw new AppError('Employee profile not found', 404);
    }

    const employeeId = payload.category === 'self' ? reviewer._id : payload.employeeId;

    // Manager remarks on an employee's progress can only be written by that employee's direct manager.
    if (payload.category === 'manager') {
      if (!payload.employeeId) {
        throw new AppError('An employee must be selected for manager remarks', 400);
      }
      const target = await EmployeeModel.findOne({ _id: payload.employeeId, isDeleted: false }).select('reportingTo');
      if (!target) {
        throw new AppError('Employee not found', 404);
      }
      if (!target.reportingTo || target.reportingTo.toString() !== reviewer.id) {
        throw new AppError('Only the direct manager can record progress remarks for this employee', 403);
      }
    }

    return FeedbackModel.create({ ...payload, reviewerId: reviewer._id, employeeId });
  }
