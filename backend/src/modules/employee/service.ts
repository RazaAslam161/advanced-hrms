...
const DEMO_MANAGER_EMAIL = 'manager.portal@metalabstech.com';
const DEMO_EMPLOYEE_EMAIL = 'employee.portal@metalabstech.com';

export const syncManagerReportingLine = async (): Promise<void> => {
  const manager = await EmployeeModel.findOne({ email: DEMO_MANAGER_EMAIL, isDeleted: false }).select('_id department');
  if (!manager) {
    return;
  }

  // Link the demo employee to the demo manager if it has no manager yet.
  await EmployeeModel.updateOne(
    { email: DEMO_EMPLOYEE_EMAIL, isDeleted: false, reportingTo: { $in: [null, undefined] } },
    { reportingTo: manager._id },
  );

  // Give the manager a small team from their own department so "My Team" is not empty.
  if (manager.department) {
    const unassigned = await EmployeeModel.find({
      department: manager.department,
      isDeleted: false,
      _id: { $ne: manager._id },
      reportingTo: { $in: [null, undefined] },
    })
      .select('_id')
      .limit(8);
    ...
    if (unassigned.length > 0) {
      await EmployeeModel.updateMany(
        { _id: { $in: unassigned.map((member) => member._id) } },
        { reportingTo: manager._id },
      );
    }
  }
...
export class EmployeeService {
  ...
  static async list(
    ...
    if (requester?.role === 'manager') {
      ...
      teamFilter = { reportingTo: manager._id };
    }
  ...
  static async uploadAvatar(id: string, file: Express.Multer.File, actorId?: string) {
    ...
    employee.avatar = await persistAvatarDataUrl(file.buffer);
    await employee.save();
