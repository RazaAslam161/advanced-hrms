...
export const analyticsLive = asyncHandler(async (_req: Request, res: Response) => {
  const live = await AnalyticsService.liveActivity();
  res.json(sendSuccess('Live activity fetched successfully', live));
});

export const analyticsLiveDetail = asyncHandler(async (req: Request, res: Response) => {
  const detail = await AnalyticsService.liveActivityDetail(String(req.params.category));
  res.json(sendSuccess('Live activity detail fetched successfully', detail));
});
