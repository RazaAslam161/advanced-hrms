...
export const Navbar = ({ portal }: { portal: PortalConfig }) => {
  ...
  return (
    <div className="relative z-40 flex flex-col gap-4 rounded-[1.35rem] border border-[color:var(--color-card-border)] bg-[color:var(--color-card-bg)] px-4 py-3.5 shadow-panel backdrop-blur-sm xl:flex-row xl:items-center xl:justify-between">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.26em] theme-muted">
          <span>{companyProfile.legalName}</span>
          <span className="h-1 w-1 rounded-full bg-[color:var(--color-muted)]" />
          <span>{portal.label}</span>
        </div>
        <h2 className="mt-2 text-[1.75rem] font-semibold text-[color:var(--color-heading)]">{current}</h2>
      </div>
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          <div ref={dropdownRef} className="relative">
            <button
              ...
              className="flex items-center gap-2 rounded-full border border-[color:var(--color-border)] bg-[color:var(--color-subtle-bg)] px-2 py-1.5 text-left transition hover:bg-[color:var(--color-input-bg)]"
            ...
            {menuOpen ? (
              <div className="absolute right-0 top-[calc(100%+0.65rem)] z-[90] w-64 rounded-2xl border border-[color:var(--color-card-border)] bg-[color:var(--color-card-bg)] p-2 shadow-2xl backdrop-blur-sm">
                <div className="flex items-center gap-3 rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-subtle-bg)] px-3 py-3">
                  <UserAvatar
                    name={currentProfileName}
                    src={profileQuery.data?.avatar}
                    active={authUser?.isActive ?? true}
                  ...
                  />
                  <div className="min-w-0">
                    <p className="truncate font-medium text-[color:var(--color-text)]">{currentProfileName}</p>
                    <p className="truncate text-sm theme-muted">{user?.email}</p>
                  </div>
                </div>
                ...
                <div className="mt-2 space-y-1">
                  <button
                    type="button"
                    className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-[color:var(--color-text)] transition hover:bg-[color:var(--color-subtle-bg)]"
