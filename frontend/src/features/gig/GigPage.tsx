...
export const GigPage = () => {
  ...
  if (gigsQuery.isLoading || meQuery.isLoading) {
    return <LoadingState label="Loading internal gig marketplace..." />;
  }

  if (gigsQuery.isError || meQuery.isError) {
      ...
      {canManageGigs ? (
        <Card className="space-y-4">
          ...
          <form
            ...
            onSubmit={form.handleSubmit((values) => saveGigMutation.mutate(values))}
          >
            ...
            render: (item) => {
              ...
              return canManageGigs ? (
                <div className="flex flex-wrap gap-2">
                  <Button type="button" variant="outline" soundTone="none" onClick={() => startEditing({ _id: item._id, title: item.title, description: item.description, status: item.status })}>
                    Edit
                  </Button>
