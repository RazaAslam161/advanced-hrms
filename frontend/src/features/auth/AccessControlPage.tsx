...
import { invalidateEmployeeDirectory } from '../../lib/employeeDirectory';
...
export const AccessControlPage = () => {
  ...
  const deleteError = deleteUserMutation.isError ? getApiErrorMessage(deleteUserMutation.error, 'User could not be removed from the company registry.') : null;
      ...
      <div className="space-y-6">
        <Card className="space-y-4">
          ...
          <form className="grid gap-3 md:grid-cols-2" onSubmit={accountForm.handleSubmit((values) => createAccountMutation.mutate(values))}>
            ...
            <select className="field-select" {...accountForm.register('role')}>
            ...
            <select className="field-select" {...accountForm.register('department')}>
          ...
          {auth.user?.role === 'superAdmin' ? (
            <div className="rounded-[1.4rem] border border-white/10 bg-white/5 p-5">
              ...
              <form className="mt-4 grid gap-3" onSubmit={transferForm.handleSubmit((values) => transferMutation.mutate(values))}>
                <select className="field-select" {...transferForm.register('targetUserId')}>
              ...
              render: (item) => (
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    disabled={deleteUserMutation.isPending || item.role === 'superAdmin' || item.id === auth.user?.id}
                    onClick={() => {
                      if (!window.confirm(`Remove ${item.firstName} ${item.lastName} from the company registry? Their login and employee profile will be archived.`)) {
                        return;
                      }
